...
if nargin == 0
   ...
   h = uicontrol('Style','popup','Position',[15,10,90,21],...
      ...
      'string',{'R''G''B''','Y''PbPr','HSV','HSL','L*a*b*','L*u*v*','L*ch'});
...
case 3
   ...
   coloraxis('V',3);
case 4
   [x,y,z,Tri] = makeshape('Double Cone');
...
case 6
   [x,y,z,Tri] = makeshape('Blobs');
   CData = colorspace('rgb<-luv',[z*100,x*100,y*100]);
   myplot(x,y,2*z,Tri,CData);
   coloraxis('L*',4);
   coloraxis('u*',5,2); 
   coloraxis('v*',6,2);
...
case 1
   ...
   x = cos(t)*1.1;
   y = sin(t)*1.1;
   set(plot3(x,y,zeros(size(x)),'k:'),'LineWidth',2.5);
   x = [x,-0.1,0,-0.1];
   y = [y,-1.05,-1.1,-1.15];
