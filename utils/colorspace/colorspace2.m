function varargout = colorspace2(Conversion,varargin)
...
% w/ additions by Shinji Nishimoto & Michael Oliver
...
case {'rgb','xyz','hsv','hsl','lab','luv','lch', 'lch2', 'lch3','lms','dkl'}
   T = Space;
otherwise
...
function Image = lch2(Image,SrcSpace)
% Convert to CIE L*c'h (skewed L*ch, two-cones)
%  Shinji Nishimoto 02/23/2009
Image = lab(Image,SrcSpace);  % Convert to CIE L*ab
H = atan2(Image(:,:,3),Image(:,:,2));
H = H*180/pi + 360*(H < 0);
Image(:,:,2) = sqrt(Image(:,:,2).^2 + Image(:,:,3).^2);  % C
...
Image(:,:,2) = Image(:,:,2).*(1-(abs(Image(:,:,1)-50)/50));

return;

function Image = lch3(Image,SrcSpace)
% Convert to CIE L*c'h (skewed L*ch, spherical)
%  Shinji Nishimoto 02/23/2009
Image = lab(Image,SrcSpace);  % Convert to CIE L*ab
H = atan2(Image(:,:,3),Image(:,:,2));
H = H*180/pi + 360*(H < 0);
...
Image(:,:,2) = Image(:,:,2).*sqrt(1-(abs(Image(:,:,1)-50)/50).^2);

return;


function Image = lms(Image,SrcSpace)
% Convert to LMS color space using Michael Oliver's code
%  Shinji Nishimoto 03/06/2009
[t,g] = pre_rgb_to_lms(2);
Image = rgb_to_lms(Image, t,g);
return;

function Image = dkl(Image,SrcSpace)
% Convert to DKL color space using Michael Oliver's code
%  Shinji Nishimoto 03/06/2009
[t,g] = pre_rgb_to_lms(2);
Image = rgb_to_dkl(Image, t,g);
return;
